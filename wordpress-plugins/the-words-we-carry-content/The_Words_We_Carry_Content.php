 ...
 * Description: Magazine content, issue, chapter, page, article, media-reference, and rights-data plugin for The Words We Carry. Ads/sponsors and analytics are intentionally separate plugins.
 * Version:     2.1.0
...
define( 'TWWC_CONTENT_VERSION', '2.1.0' );
...
define( 'TWWC_CONTENT_PUBLIC_ISSUE_URL', 'https://raw.githubusercontent.com/Joliel21/bta_public/main/public/content/issue.json' );
...
add_action( 'admin_menu', 'twwc_content_register_import_page', 11 );
add_action( 'admin_post_twwc_content_import_github_articles', 'twwc_content_handle_github_article_import' );
...
function twwc_content_default_settings() {
	return array(
		...
		'public_privacy_url'      => '',
		'public_terms_url'        => '',
		'public_issue_url'        => TWWC_CONTENT_PUBLIC_ISSUE_URL,
	);
}
...
function twwc_content_sanitize_settings( $input ) {
	...
	foreach ( $defaults as $key => $default ) {
		...
		if ( in_array( $key, array( 'public_permissions_url', 'public_privacy_url', 'public_terms_url', 'public_issue_url' ), true ) ) {
...
function twwc_content_render_settings_page() {
	...
	<div class="wrap">
		...
		<p>This plugin owns magazine issues, chapters, articles, pages, image references, CTA content, placement metadata, and rights metadata. Ads/sponsors and analytics are intentionally separate plugins.</p>
		...
		<h2>GitHub issue source</h2>
		<p>This URL is used for importing/updating WordPress article records while keeping GitHub as the magazine structure source.</p>
		<code><?php echo esc_html( $settings['public_issue_url'] ); ?></code>

		<form method="post" action="options.php" style="margin-top: 24px;">
			<?php settings_fields( 'twwc_content_settings_group' ); ?>
			<table class="form-table" role="presentation">
				<tr>
					<th scope="row"><label for="twwc_default_issue_id">Default issue ID</label></th>
				...
				<tr>
					<th scope="row"><label for="twwc_public_issue_url">GitHub issue.json URL</label></th>
					<td><input id="twwc_public_issue_url" class="large-text code" name="<?php echo esc_attr( $key ); ?>[public_issue_url]" value="<?php echo esc_attr( $settings['public_issue_url'] ); ?>" /></td>
				</tr>
				<tr>
	...
	register_post_type(
		...
		array(
			'labels'       => array(
				...
				'add_new_item'  => 'Add Magazine Issue',
...
function twwc_content_render_page_meta_box( $post ) {
	...
	<div class="twwc-content-meta-grid">
		...
		<label for="_twwc_issue_id">Issue ID</label>
...
function twwc_content_render_issue_meta_box( $post ) {
	...
	<div class="twwc-issue-meta-grid">
		...
		<label for="_twwc_issue_status">Issue status</label>
		...
		<label for="_twwc_issue_fingerprint">Issue fingerprint</label>
		...
		<label for="_twwc_issue_rights_note">Issue rights note</label>
...
function twwc_content_sanitize_page_meta_value( $key, $value ) {
	if ( in_array( $key, array( '_twwc_button_url', '_twwc_image_click_url', '_twwc_embed_url', '_twwc_audio_url', '_twwc_transcript_url' ), true ) ) {
...
function twwc_content_rest_magazine( WP_REST_Request $request ) {
	...
	if ( is_array( $github_payload ) ) {
		return rest_ensure_response( twwc_content_merge_wordpress_articles_with_github_issue( $github_payload, $pages, $issue_id, $issue ) );
	}
	...
	return rest_ensure_response(
		array(
			'schemaVersion' => '1.0.0',
			'source'        => 'wordpress-only-no-github-issue',
			'generatedAt'   => gmdate( 'c' ),
			'issueId'       => $issue_id,
			'issue'         => $issue,
...
function twwc_content_get_github_payload( $issue_id, $issue ) {
	$settings = twwc_content_get_settings();
	$issue_url = ! empty( $settings['public_issue_url'] ) ? $settings['public_issue_url'] : TWWC_CONTENT_PUBLIC_ISSUE_URL;
...
function twwc_content_normalize_page_post( $post ) {
	...
	return array(
		...
		'image'              => $thumb_id ? twwc_content_media_payload( $thumb_id, get_the_title( $post ) ) : null,
		'imageUrl'           => $thumb_id ? wp_get_attachment_image_url( $thumb_id, 'full' ) : '',
		'imageAlt'           => $thumb_id ? get_post_meta( $thumb_id, '_wp_attachment_image_alt', true ) : get_the_title( $post ),
		'imageCaption'       => $thumb_id ? wp_get_attachment_caption( $thumb_id ) : '',
		'imageLinkUrl'       => twwc_content_get_page_meta( $post_id, '_twwc_image_click_url', '' ),
		'images'             => twwc_content_page_images_payload( $post_id, $thumb_id, get_the_title( $post ) ),
...
function twwc_content_media_payload( $attachment_id, $fallback_alt = '' ) {
	...
	return array(
		...
		'description' => $image_post ? $image_post->post_content : '',
		'title'       => $image_post ? $image_post->post_title : '',
		'slug'        => $image_post ? $image_post->post_name : '',
...
function twwc_content_issue_columns( $columns ) {
	$columns['twwc_issue_id'] = 'Issue ID';
...
function twwc_content_register_import_page() {
	add_submenu_page(
		'twwc-content',
		'Import GitHub Articles',
		'Import GitHub Articles',
		'manage_options',
		'twwc-content-import-github',
		'twwc_content_render_import_page'
	);
}

function twwc_content_render_import_page() {
	if ( ! current_user_can( 'manage_options' ) ) {
		return;
	}

	$settings = twwc_content_get_settings();
	$last = get_option( 'twwc_content_last_github_import', array() );
	?>
	<div class="wrap">
		<h1>Import GitHub Articles to WordPress</h1>
		<p>This imports article records from <code>issue.json</code> into WordPress magazine content posts. Each WordPress post slug and reader content ID will match the GitHub article ID.</p>
		<p><strong>Source:</strong> <code><?php echo esc_html( $settings['public_issue_url'] ); ?></code></p>
		<?php if ( is_array( $last ) && ! empty( $last ) ) : ?>
			<h2>Last import</h2>
			<pre style="max-width:900px;white-space:pre-wrap;background:#fff;border:1px solid #ccd0d4;padding:12px;"><?php echo esc_html( wp_json_encode( $last, JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES ) ); ?></pre>
		<?php endif; ?>
		<form method="post" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>">
			<?php wp_nonce_field( 'twwc_content_import_github_articles', 'twwc_content_import_nonce' ); ?>
			<input type="hidden" name="action" value="twwc_content_import_github_articles" />
			<?php submit_button( 'Import / Update WordPress Articles from GitHub issue.json', 'primary large' ); ?>
		</form>
		<p>This importer does not delete WordPress posts. It creates or updates matching article posts only.</p>
	</div>
	<?php
}

function twwc_content_handle_github_article_import() {
	if ( ! current_user_can( 'manage_options' ) ) {
		wp_die( esc_html__( 'You do not have permission to import articles.', 'the-words-we-carry-content' ) );
	}

	if ( ! isset( $_POST['twwc_content_import_nonce'] ) || ! wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['twwc_content_import_nonce'] ) ), 'twwc_content_import_github_articles' ) ) {
		wp_die( esc_html__( 'Invalid import request.', 'the-words-we-carry-content' ) );
	}

	$result = twwc_content_import_github_articles();
	update_option( 'twwc_content_last_github_import', $result, false );
	wp_safe_redirect( add_query_arg( array( 'page' => 'twwc-content-import-github', 'twwc-imported' => '1' ), admin_url( 'admin.php' ) ) );
	exit;
}

function twwc_content_import_github_articles() {
	$settings = twwc_content_get_settings();
	$issue_url = ! empty( $settings['public_issue_url'] ) ? $settings['public_issue_url'] : TWWC_CONTENT_PUBLIC_ISSUE_URL;
	$issue = twwc_content_fetch_public_json( $issue_url );

	if ( ! is_array( $issue ) || empty( $issue['articles'] ) || ! is_array( $issue['articles'] ) ) {
		return array(
			'ok' => false,
			'error' => 'GitHub issue.json could not be loaded or contains no articles.',
			'issueUrl' => $issue_url,
			'generatedAt' => gmdate( 'c' ),
		);
	}

	$base_raw_url = isset( $issue['source']['baseRawUrl'] ) ? trailingslashit( $issue['source']['baseRawUrl'] ) : 'https://raw.githubusercontent.com/Joliel21/bta_public/main/public/';
	$issue_id = ! empty( $issue['issueId'] ) ? sanitize_text_field( $issue['issueId'] ) : $settings['default_issue_id'];
	$created = 0;
	$updated = 0;
	$skipped = 0;
	$errors = array();
	...
	foreach ( $issue['articles'] as $index => $article ) {
		...
		$article_id = sanitize_title( $article['id'] );
		$title = ! empty( $article['title'] ) ? wp_strip_all_tags( $article['title'] ) : $article_id;
		$markdown = '';
		if ( ! empty( $article['markdownPath'] ) ) {
			$markdown_url = twwc_content_resolve_public_asset_url( $base_raw_url, $article['markdownPath'] );
			$response = wp_remote_get( $markdown_url, array( 'timeout' => 15, 'redirection' => 3 ) );
			if ( ! is_wp_error( $response ) && 200 === (int) wp_remote_retrieve_response_code( $response ) ) {
				$markdown = wp_remote_retrieve_body( $response );
			} else {
				$errors[] = 'Could not load markdown for ' . $article_id;
			}
		}

		$post_id = twwc_content_find_post_by_content_id_or_slug( $article_id );
		$post_data = array(
			'post_type'    => TWWC_CONTENT_POST_TYPE,
			'post_status'  => 'publish',
			'post_title'   => $title,
			'post_name'    => $article_id,
			'post_excerpt' => ! empty( $article['subtitle'] ) ? wp_strip_all_tags( $article['subtitle'] ) : '',
		...
		if ( $post_id ) {
			...
			if ( is_wp_error( $result ) ) {
				$errors[] = $article_id . ': ' . $result->get_error_message();
				continue;
			}
			$updated++;
		} else {
		...
		update_post_meta( $post_id, '_twwc_content_id', $article_id );
		update_post_meta( $post_id, '_twwc_issue_id', $issue_id );
		update_post_meta( $post_id, '_twwc_page_type', 'article' );
		update_post_meta( $post_id, '_twwc_layout_id', ! empty( $article['layoutId'] ) ? sanitize_text_field( $article['layoutId'] ) : 'article-text-layout' );
		update_post_meta( $post_id, '_twwc_source_priority', 'wordpress' );
		update_post_meta( $post_id, '_twwc_analytics_label', $title );

		if ( ! empty( $article['date'] ) ) {
			update_post_meta( $post_id, '_twwc_article_date', sanitize_text_field( $article['date'] ) );
		}

		if ( ! empty( $article['chapterSlug'] ) ) {
			$term_slug = sanitize_title( $article['chapterSlug'] );
			$term_name = ! empty( $article['chapter'] ) ? wp_strip_all_tags( $article['chapter'] ) : $term_slug;
			$term = term_exists( $term_slug, TWWC_CONTENT_CHAPTER_TAXONOMY );
			if ( ! $term ) {
				$term = wp_insert_term( $term_name, TWWC_CONTENT_CHAPTER_TAXONOMY, array( 'slug' => $term_slug ) );
			}
			if ( ! is_wp_error( $term ) ) {
				wp_set_object_terms( $post_id, array( $term_slug ), TWWC_CONTENT_CHAPTER_TAXONOMY, false );
			}
		}

		$image_ids = twwc_content_match_article_images_to_media_library( $article );
		if ( ! empty( $image_ids ) ) {
			set_post_thumbnail( $post_id, $image_ids[0] );
			update_post_meta( $post_id, '_twwc_wp_image_ids_json', wp_json_encode( array_values( array_unique( array_map( 'absint', $image_ids ) ) ) ) );
			$mapped_images[ $article_id ] = $image_ids;
		} else {
			delete_post_meta( $post_id, '_twwc_wp_image_ids_json' );
		}
	}
	...
	return array(
		...
		'skipped' => $skipped,
		'errors' => $errors,
		'mappedImages' => $mapped_images,
		'generatedAt' => gmdate( 'c' ),
	);
}

function twwc_content_find_post_by_content_id_or_slug( $article_id ) {
	$article_id = sanitize_title( $article_id );
	$query = new WP_Query(
		array(
			'post_type'      => TWWC_CONTENT_POST_TYPE,
	...
	$meta_query = new WP_Query(
		array(
			...
			'posts_per_page' => 1,
			'no_found_rows'  => true,
			'meta_key'       => '_twwc_content_id',
			'meta_value'     => $article_id,
		)
	);

	if ( ! empty( $meta_query->posts ) ) {
		$post_id = (int) $meta_query->posts[0]->ID;
		wp_reset_postdata();
		return $post_id;
	}
	wp_reset_postdata();
...
function twwc_content_match_article_images_to_media_library( $article ) {
	$ids = array();
	$images = isset( $article['images'] ) && is_array( $article['images'] ) ? $article['images'] : array();

	foreach ( $images as $image ) {
		$id = twwc_content_find_attachment_for_image_record( $image );
		if ( $id ) {
			$ids[] = $id;
		}
	}
...
function twwc_content_find_attachment_for_image_record( $image ) {
	$candidates = array();
	foreach ( array( 'wordpressMediaSlug', 'slug', 'title', 'filename', 'file', 'src', 'url' ) as $key ) {
		if ( ! empty( $image[ $key ] ) ) {
			$value = (string) $image[ $key ];
			$candidates[] = $value;
			$candidates[] = wp_basename( strtok( $value, '?' ) );
			$candidates[] = preg_replace( '/\.[a-z0-9]+$/i', '', wp_basename( strtok( $value, '?' ) ) );
		}
	}

	$candidate_slugs = array_values( array_unique( array_filter( array_map( 'sanitize_title', $candidates ) ) ) );
	...
	foreach ( $query->posts as $attachment ) {
		...
		$attachment_candidates = array(
			$attachment->post_name,
			$attachment->post_title,
			$path ? wp_basename( $path ) : '',
			$path ? preg_replace( '/\.[a-z0-9]+$/i', '', wp_basename( $path ) ) : '',
		);
		$attachment_slugs = array_values( array_unique( array_filter( array_map( 'sanitize_title', $attachment_candidates ) ) ) );
		if ( array_intersect( $candidate_slugs, $attachment_slugs ) ) {
			wp_reset_postdata();
...
function twwc_content_resolve_public_asset_url( $base_raw_url, $path ) {
	$path = ltrim( preg_replace( '#^public/#', '', (string) $path ), '/' );
	if ( preg_match( '#^https?://#i', $path ) ) {
		return $path;
	}
	return trailingslashit( $base_raw_url ) . $path;
}

function twwc_content_page_images_payload( $post_id, $thumb_id = 0, $fallback_alt = '' ) {
	$ids = array();
	if ( $thumb_id ) {
		$ids[] = absint( $thumb_id );
	}

	$stored = get_post_meta( $post_id, '_twwc_wp_image_ids_json', true );
	$decoded = $stored ? json_decode( $stored, true ) : array();
	if ( is_array( $decoded ) ) {
	...
	return array_values( array_filter( array_map( function( $id ) use ( $fallback_alt ) {
		return twwc_content_media_payload( $id, $fallback_alt );
	}, $ids ) ) );
}

function twwc_content_merge_wordpress_articles_with_github_issue( $github_payload, $wordpress_pages, $issue_id, $issue ) {
	$wordpress_articles = array_values( array_filter( $wordpress_pages, 'twwc_content_is_article_payload' ) );
	$wordpress_by_id = array();
	foreach ( $wordpress_articles as $article ) {
		if ( ! empty( $article['id'] ) ) {
			$wordpress_by_id[ sanitize_title( $article['id'] ) ] = $article;
		}
		if ( ! empty( $article['wordpressSlug'] ) ) {
			$wordpress_by_id[ sanitize_title( $article['wordpressSlug'] ) ] = $article;
		}
	}

	$github_articles = isset( $github_payload['articles'] ) && is_array( $github_payload['articles'] ) ? $github_payload['articles'] : array();
	$merged_articles = array();
	foreach ( $github_articles as $github_article ) {
		$id = ! empty( $github_article['id'] ) ? sanitize_title( $github_article['id'] ) : '';
		if ( $id && isset( $wordpress_by_id[ $id ] ) ) {
			$wp_article = $wordpress_by_id[ $id ];
			$merged_articles[] = array_merge( $github_article, $wp_article, array(
				'id' => $id,
				'wordpressSlug' => ! empty( $wp_article['wordpressSlug'] ) ? $wp_article['wordpressSlug'] : $id,
				'githubFallback' => $github_article,
				'source' => 'wordpress-article-github-structure',
			) );
		} else {
			$github_article['wordpressSlug'] = $id;
			$github_article['source'] = ! empty( $github_article['source'] ) ? $github_article['source'] : 'github-fallback';
			$merged_articles[] = $github_article;
		}
	}

	$github_payload['schemaVersion'] = '2.1.0';
	$github_payload['source'] = 'wordpress-articles-github-structure';
	$github_payload['generatedAt'] = gmdate( 'c' );
	$github_payload['issueId'] = $issue_id;
	$github_payload['issue'] = isset( $github_payload['issue'] ) ? $github_payload['issue'] : $issue;
	$github_payload['rights'] = twwc_content_get_rights_payload( $issue );
	$github_payload['articles'] = $merged_articles;
	$github_payload['pages'] = $merged_articles;
	$github_payload['wordpressArticles'] = $wordpress_articles;
	$github_payload['api'] = array(
		'content' => rest_url( TWWC_CONTENT_REST_NAMESPACE . '/magazine' ),
		'import' => admin_url( 'admin.php?page=twwc-content-import-github' ),
	);
