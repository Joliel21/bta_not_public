...
define( 'TWWC_CONTENT_VERSION', '2.1.3' );
...
function twwc_content_rest_magazine( WP_REST_Request $request ) {
	...
	$github_issue    = twwc_content_get_public_issue_json();
	$wp_articles     = array_values( array_filter( $pages, 'twwc_content_is_article_payload' ) );
	$wp_front_matter = array_values( array_filter( $pages, 'twwc_content_is_front_matter_payload' ) );
	$wp_back_matter  = array_values( array_filter( $pages, 'twwc_content_is_back_matter_payload' ) );

	// Emergency mode only: if WordPress has no published article/content pages,
	// fall back to the public GitHub issue archive so the reader does not go blank.
	if ( empty( $pages ) ) {
	...
	$chapters             = twwc_content_get_chapters_for_pages( $pages, $github_issue );
	$chapter_descriptions = twwc_content_get_chapter_descriptions_payload( $github_issue );
	$front_matter         = ! empty( $wp_front_matter ) ? $wp_front_matter : twwc_content_get_github_front_matter_pages( $github_issue );
	$back_matter          = ! empty( $wp_back_matter ) ? $wp_back_matter : twwc_content_get_github_back_matter_pages( $github_issue );

	return rest_ensure_response(
		array(
			'schemaVersion'       => '2.1.3',
			'source'              => 'wordpress-content-with-github-shell',
			'generatedAt'         => gmdate( 'c' ),
			'issueId'             => $issue_id,
			'issue'               => $issue,
			...
			'pages'               => $pages,
			'frontMatter'         => array( 'pages' => $front_matter ),
			...
			'magazineManifest'    => $github_issue,
			'issueJson'           => $github_issue,
			'contentMode'         => array(
				'liveArticles' => 'wordpress',
				'shell'        => 'github-issue-json',
				'fallback'     => 'github-only-if-wordpress-empty-or-api-fails',
			),
			'api'                 => array(
...
function twwc_content_get_public_issue_json() {
	...
	return is_array( $issue_json ) ? $issue_json : array();
}

function twwc_content_get_github_front_matter_pages( $issue_json ) {
	return isset( $issue_json['frontMatterPages'] ) && is_array( $issue_json['frontMatterPages'] )
		? array_values( $issue_json['frontMatterPages'] )
		: array();
}

function twwc_content_get_github_back_matter_pages( $issue_json ) {
	return isset( $issue_json['backMatterPages'] ) && is_array( $issue_json['backMatterPages'] )
		? array_values( $issue_json['backMatterPages'] )
		: array();
}

function twwc_content_get_chapter_descriptions_payload( $issue_json ) {
	$descriptions = array();
	if ( ! isset( $issue_json['chapterDescriptions'] ) || ! is_array( $issue_json['chapterDescriptions'] ) ) {
		return $descriptions;
	}

	foreach ( $issue_json['chapterDescriptions'] as $description ) {
		if ( ! is_array( $description ) ) {
			continue;
		}

		$slug = isset( $description['slug'] ) ? sanitize_title( $description['slug'] ) : '';
		if ( '' === $slug ) {
			$slug = isset( $description['id'] ) ? sanitize_title( $description['id'] ) : '';
		}
...
function twwc_content_get_chapters_for_pages( $pages, $issue_json ) {
	$wordpress_chapters = twwc_content_get_chapters();
	$articles_by_chapter = array();

	foreach ( $pages as $page ) {
		if ( ! twwc_content_is_article_payload( $page ) ) {
			continue;
		}

		$chapter_slug = isset( $page['chapterSlug'] ) ? sanitize_title( $page['chapterSlug'] ) : 'wordpress';
		if ( ! isset( $articles_by_chapter[ $chapter_slug ] ) ) {
			$articles_by_chapter[ $chapter_slug ] = array();
		}

		if ( ! empty( $page['id'] ) ) {
			$articles_by_chapter[ $chapter_slug ][] = $page['id'];
		}
	}

	$chapter_by_slug = array();
	foreach ( $wordpress_chapters as $chapter ) {
		$slug = isset( $chapter['slug'] ) ? sanitize_title( $chapter['slug'] ) : '';
		if ( '' !== $slug ) {
			$chapter['articleIds'] = isset( $articles_by_chapter[ $slug ] ) ? array_values( $articles_by_chapter[ $slug ] ) : array();
	...
	if ( isset( $issue_json['chapters'] ) && is_array( $issue_json['chapters'] ) ) {
		foreach ( $issue_json['chapters'] as $github_chapter ) {
			if ( ! is_array( $github_chapter ) ) {
				continue;
			}

			$slug = isset( $github_chapter['slug'] ) ? sanitize_title( $github_chapter['slug'] ) : '';
			if ( '' === $slug ) {
				continue;
			}

			if ( isset( $chapter_by_slug[ $slug ] ) ) {
				$merged = wp_parse_args( $chapter_by_slug[ $slug ], $github_chapter );
				$merged['articleIds'] = $chapter_by_slug[ $slug ]['articleIds'];
				$merged['source'] = 'wordpress-with-github-order';
				$ordered[] = $merged;
				unset( $chapter_by_slug[ $slug ] );
			} elseif ( isset( $articles_by_chapter[ $slug ] ) ) {
				$github_chapter['articleIds'] = array_values( $articles_by_chapter[ $slug ] );
				$github_chapter['source'] = 'github-shell-wordpress-articles';
				$ordered[] = $github_chapter;
			}
		}
	}

	foreach ( $chapter_by_slug as $chapter ) {
		if ( ! empty( $chapter['articleIds'] ) ) {
			$ordered[] = $chapter;
		}
	}
